...
| **VS Code extension** | `pnpm run sync:media` in the monorepo copies `brand/` → `apps/vscode-extension/media/` for the VSIX |
| **Marketplace README** | `screenshots/` via `raw.githubusercontent.com` URLs |

**To change the logo:** edit `brand/icon.svg` here, regenerate PNGs (`pnpm --filter a11y-studio run icon:render` in the monorepo), commit and push **this repo**.
